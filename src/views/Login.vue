    ...
    <div class="relative flex flex-1 flex-col items-center justify-center pt-12 pb-16"><svg
        ...
        <form action="/login" class="w-full max-w-sm">
            <div class="mb-6"><label for="username"
                    class="block text-sm font-semibold leading-6 text-gray-900">Username</label><input
            ...
            <div class="mb-6"><label for="password"
                    ...
                    id="password"
                    class="mt-2 appearance-none text-slate-900 bg-white rounded-md block w-full px-3 h-10 shadow-sm sm:text-sm focus:outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-sky-500 ring-1 ring-slate-200"
                    v-model="password"></div><button type="submit" @click.prevent="handleLogin()"
...
const handleLogin = async () => {
    ...
    await login(username.value, password.value).then(success => {
        const authStore = useAuth()
        authStore.setAuth(success.data)
        router.push({
            name: 'home',
        })
    })
